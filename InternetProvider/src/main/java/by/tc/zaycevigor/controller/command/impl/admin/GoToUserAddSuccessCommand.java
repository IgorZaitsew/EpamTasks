...
import static by.tc.zaycevigor.controller.command.util.JspPageName.USER_ADD_SUCCESS_PAGE;

public class GoToUserAddSuccessCommand implements Command {
    @Override
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String url = CreatorFullURL.create(request);
        request.getSession(true).setAttribute(PREV_REQUEST, url);
        RequestDispatcher dispatcher = request.getRequestDispatcher(USER_ADD_SUCCESS_PAGE);
