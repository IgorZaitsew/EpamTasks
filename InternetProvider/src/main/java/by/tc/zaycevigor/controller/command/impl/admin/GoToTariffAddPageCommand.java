...
import static by.tc.zaycevigor.controller.command.util.JspPageName.TARIFF_ADD_PAGE;

public class GoToTariffAddPageCommand implements Command {
    @Override
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String url = CreatorFullURL.create(request);
        request.getSession(true).setAttribute(PREV_REQUEST, url);
        RequestDispatcher dispatcher = request.getRequestDispatcher(TARIFF_ADD_PAGE);
